...
type historicalActivationState struct {
	...
	// choiceID is the specific winning choiceID for the associated agenda.
	choiceID string
}
...
type consensusAgenda struct {
	...
	//
	// It is only populated when the associated chain parameters specify the
	// necessary data and no forced state is specified (aka is nil).
...
func makeAgendas(params *chaincfg.Params, historicalAgendas map[wire.CurrencyNet]historicalAgenda) (map[string]*consensusAgenda, error) {
	...
	for id, histAgenda := range historicalAgendas[params.Net] {
		...
		if info.forcedState != nil {
			str := fmt.Sprintf("deployment ID %s has both a forced choice and "+
				"a historical state configured", id)
