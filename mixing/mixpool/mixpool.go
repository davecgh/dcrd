...
// removeOrphansBySource removes up to the maximum specified number of orphan
// messages associated with the provided source and returns the number of
// entries removed.

// This function MUST be called with the mixpool lock held (for writes).
func (p *Pool) removeOrphansBySource(src Source, maxToEvict uint64) uint64 {
	var numEvicted uint64
	for hash, orphan := range p.orphans {
		if numEvicted >= maxToEvict {
			break
		}
		if orphan.src.ID() == src.ID() {
...
func (p *Pool) limitNumOrphans() {
	...
	sourcesByID := make(map[uint64]Source)
	for _, orphan := range p.orphans {
		srcID := orphan.src.ID()
		sourcesByID[srcID] = orphan.src
		srcCounters[srcID]++
	}
	...
	for srcID, count := range srcCounters {
		srcCounts = append(srcCounts, srcWithCount{sourcesByID[srcID], count})
	}
	...
	for numOrphans > maxPostEvictionOrphans && len(srcCounts) > 0 {
		...
		numEvicted := p.removeOrphansBySource(src, maxToEvict)
		log.Tracef("Removed %d orphans with source ID %d", numEvicted, src.ID())
